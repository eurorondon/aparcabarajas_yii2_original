...
class ReservasController extends Controller
{
    ...
    public function actionAgencia()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $clientes->load(Yii::$app->request->post()) && $coches->load(Yii::$app->request->post())) {
            ...
            $clientes->tipo_documento = 'S/N';
            $clientes->nro_documento = 'S/N';
            $clientes->correo = $randomString . '@aparcabarajas.es';
            $clientes->estatus = 1;
            $clientes->created_at = $fecha_creacion;
            $clientes->created_by = $id_usuario;
            $clientes->save();
            ...
            foreach ($servicios as $ser) {
                ...
                $cantidad = empty($_POST['cantidad' . $ser->id]) ? 0 : $_POST['cantidad' . $ser->id];
                $precio_total = empty($_POST['precio_total' . $ser->id]) ? 0 : $_POST['precio_total' . $ser->id];
            ...
            $model->id_cliente = $id_cliente;
            $model->id_coche = $id_coche;
            $monto_factura = $model->costo_servicios / ($iva + 1);
    ...
    protected function serviceNocturno($hora_e, $hora_s)
    {
        if (
            (date("H:i", strtotime($hora_e)) >= date("H:i", strtotime('00:30')) && date("H:i", strtotime($hora_e)) <= date("H:i", strtotime('03:45')))
            ||
            (date("H:i", strtotime($hora_s)) >= date("H:i", strtotime('00:30') && date("H:i", strtotime($hora_s)) <= date("H:i", strtotime('03:45'))))
        ) {
            return 1;
        }
    ...
    public function actionCreate()
    {
        ...
        $precioTemporada = PrecioTemporada::find()->all();
        ...
        if ($model->load(Yii::$app->request->post()) && $clientes->load(Yii::$app->request->post()) && $coches->load(Yii::$app->request->post())) {
            /*$data = Yii::$app->request->post();
                     print_r(number_format($data['Reservas']['monto_total'], 2, '.', ''));
                     print_r($data);
                     return;*/
            $medio = $_POST['medio_reserva'];
            $agencia = $_POST['agencia'];
            ...
            $montonoiva = ($model->monto_total / (1 + $iva));
            $montoimp = $montonoiva * 0.21;
            ...
            $model->monto_des = $model->descuento == 'NO' ? 0 : $model->monto_des;
    ...
    public function actionUpdate($id)
    {
        ...
        $serviciosExtras = Servicios::find()->where(['estatus' => '1'])->andWhere(['fijo' => '2'])->all();

        $idtechado = $serviciosExtras[5]->id;
    ...
    public function actionEstatus()
    {
        ...
        $reservas = Reservas::find()->where(['!=', 'estatus', '10'])->orderBy(['nro_reserva' => SORT_DESC])->all();
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($datos_reserva != null) {
                ...
                foreach ($servicios as $service) {
                    ...
                    $modelFS->precio_unitario = $service->precio_unitario / $iva;
                    $modelFS->precio_total = $service->precio_total / $iva;
