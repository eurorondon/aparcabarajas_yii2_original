...
if($tipo_afiliado == 0) {
    ...
    $totales = count(Reservas::find()->where(['!=','estatus','10'])->andWhere(['between', 'created_at', date("Y-m-d", strtotime(date('Y-m-d') . "- 3 month")), date('Y-m-d')])->all());

    $canceladas = Reservas::find()->where(['estatus' => '0'])->andWhere(['between', 'created_at', date("Y-m-d", strtotime(date('Y-m-d') . "- 3 month")), date('Y-m-d')])->all();
    $nro_can = count($canceladas);

    $pendientes = Reservas::find()->where(['estatus' => '1'])->andWhere(['between', 'created_at', date("Y-m-d", strtotime(date('Y-m-d') . "- 3 month")), date('Y-m-d')])->all();
    $nro_pen = count($pendientes);

    $finalizadas = Reservas::find()->where(['estatus' => '2'])->andWhere(['between', 'created_at', date("Y-m-d", strtotime(date('Y-m-d') . "- 3 month")), date('Y-m-d')])->all();
    $nro_fin = count($finalizadas);

    $activas = Reservas::find()->where(['estatus' => '3'])->andWhere(['between', 'created_at', date("Y-m-d", strtotime(date('Y-m-d') . "- 3 month")), date('Y-m-d')])->all();
    $nro_act = count($activas);

    $rezagadas = Reservas::find()->where(['estatus' => '4'])->andWhere(['between', 'created_at', date("Y-m-d", strtotime(date('Y-m-d') . "- 3 month")), date('Y-m-d')])->all();
    $nro_res = count($rezagadas);


} else {
    $totales = count(Reservas::find()->where(['!=','estatus','10'])->andWhere(['medio_reserva' => 4])->where(['between', 'created_at', date("Y-m-d", strtotime(date('Y-m-d') . "- 3 month")), date('Y-m-d')])->all());

    $canceladas = Reservas::find()->where(['estatus' => '0'])->andWhere(['medio_reserva' => 4])->andWhere(['between', 'created_at', date("Y-m-d", strtotime(date('Y-m-d') . "- 3 month")), date('Y-m-d')])->all();
    $nro_can = count($canceladas);

    $pendientes = Reservas::find()->where(['estatus' => '1'])->andWhere(['medio_reserva' => 4])->andWhere(['between', 'created_at', date("Y-m-d", strtotime(date('Y-m-d') . "- 3 month")), date('Y-m-d')])->all();
    $nro_pen = count($pendientes);

    $finalizadas = Reservas::find()->where(['estatus' => '2'])->andWhere(['medio_reserva' => 4])->andWhere(['between', 'created_at', date("Y-m-d", strtotime(date('Y-m-d') . "- 3 month")), date('Y-m-d')])->all();
    $nro_fin = count($finalizadas);

    $activas = Reservas::find()->where(['estatus' => '3'])->andWhere(['medio_reserva' => 4])->andWhere(['between', 'created_at', date("Y-m-d", strtotime(date('Y-m-d') . "- 3 month")), date('Y-m-d')])->all();
    $nro_act = count($activas);

    $rezagadas = Reservas::find()->where(['estatus' => '4'])->andWhere(['medio_reserva' => 4])->andWhere(['between', 'created_at', date("Y-m-d", strtotime(date('Y-m-d') . "- 3 month")), date('Y-m-d')])->all();
    $nro_res = count($rezagadas);
}
        ...
        <div class="col-lg-12 col-md-12 col-xs-12">
          <div class="panel panel-default busqueda" style="margin: 0">
            <div class="panel-body body-busqueda">
              ...
              <div class="col-lg-4">
                <div class="restotal" id="restotal">CANTIDAD DE RESERVAS ULTIMOS 3 MESES : <?= $totales ?></div>
              </div>
                    ...
                    [
                      ...
                      'value' => function ($model) {
                          if ($model->medio_reserva === 1) {
                              return Html::tag('span', '', ['style' => ['color' => '#961007'], 'class' => 'glyphicon glyphicon-earphone']);
                          }
                          if ($model->medio_reserva === 2) {
                              return Html::tag('span', '', ['style' => ['color' => '#f0ad4e'], 'class' => 'glyphicon glyphicon-bookmark']);
                          }
                          if ($model->medio_reserva === 3) {
                              return Html::tag('span', '', ['style' => ['color' => '#398439'], 'class' => 'glyphicon glyphicon-modal-window']);
                          }
