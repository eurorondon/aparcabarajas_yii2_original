     ...
     public function actionGracias()
{
    ...
    public function actionIndexp()
    {
        ...
        if (count($precioTemporada) > 0) {
            foreach ($precioTemporada as $key => $temporada) {
                $precio_diario[$key]['precio'] = $precio_diario[$key]['costo'] + ($precio_diario[$key]['cantidad'] * $temporada->precio);
            }
        }
...
public function actionContact()
{
    ...
    if (isset($postData['Reservas'])) {
        $model->attributes = $postData['Reservas'];
    }

    if ($model->validate()) {
        if ($model->sendEmail($model->email)) {
            return $this->asJson(['success' => true, 'message' => 'Gracias por contactarnos.']);
        } else {
            return $this->asJson(['success' => false, 'message' => 'Hubo un error al enviar tu mensaje.']);
        }
    }
...
public function actionContact()
{
    ...
    // Asignar manualmente los valores del formulario si existen en el request
    if (isset($postData['Reservas'])) {
        $model->attributes = $postData['Reservas'];
    }
    ...
    if ($model->validate()) {
        if ($model->sendEmail($model->email)) { 
            Yii::info('Correo enviado correctamente', __METHOD__);

            // Si es una solicitud AJAX, responder con JSON
            if (Yii::$app->request->isAjax) {
                return ['success' => true, 'message' => 'Gracias por contactarnos. Nosotros responderemos a la mayor brevedad posible.'];
            }

            // Si es una solicitud normal, redirigir a la página de agradecimiento
            Yii::$app->session->setFlash('success', 'Gracias por contactarnos. Nosotros responderemos a la mayor brevedad posible.');
            return Yii::$app->response->redirect(['site/gracias']);
        } else {
            Yii::error('Error al enviar el correo', __METHOD__);
    ...
    // Si la validación falla, devolver errores en JSON si es una petición AJAX
    if (Yii::$app->request->isAjax) {
        return [
            'success' => false,
            'message' => 'Error en la validación del formulario.',
    ...
    // Si no es AJAX, renderizar la vista normalmente
    ...
    */
        ...
        if ($model->load(Yii::$app->request->post()) && $modelC->load(Yii::$app->request->post()) && $modelV->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                ...
                if ($model->factura == 1) {
                    ...
                    $correo->setTo("asistenciaplus01@gmail.com")
                        ->setFrom([Yii::$app->params['reservasEmail'] => 'Reservas - ' . Yii::$app->name])
                        ->setSubject('Reservación Aparcabaraja con Factura')
                        ->send();
                }
    ...
    public function actionUpdate()
    {
        ...
        $model->factura = isset($_GET["invoice"]) ? $_GET["invoice"] : 0;
        ...
        if ($model->load(Yii::$app->request->post()) && $modelC->load(Yii::$app->request->post()) && $modelV->load(Yii::$app->request->post())) {
            ...
            if ($_POST['solicitud_factura']) {
                ...
                $correo->setTo("asistenciaplus01@gmail.com")
                    ->setFrom([Yii::$app->params['reservasEmail'] => 'Reservas - ' . Yii::$app->name])
                    ->setSubject('Reservación Aparcabarajas con Factura')
                    ->send();
    ...
    {
        return $this->render('_reportView', ['model' => $this->findModel(4)]);
    }
        ...
        if (Yii::$app->request->post()) {
            ...
            if ($modelReserva == null) {
                ...
                if ($modelReserva->factura == 0) {
                    ...
                    $confirmacion_correo->setTo('facturas@aparcabarajas.es')
                        ->setFrom([Yii::$app->params['reservasEmail'] => 'Facturación - ' . Yii::$app->name])
                        ->setSubject('Solicitud de Factura - Aparcabarajas')
                        ->send();
    ...
    public function actionReserva()
    {
        ...
        if (Yii::$app->request->post()) {
            $model = new Reservas();
            $model->fecha_entrada = empty($_POST['Reservas']['fechae']) ? date("d-m-Y") : $_POST['Reservas']['fechae'];
            $model->hora_entrada = empty($_POST['Reservas']['horae']) ? date('H:i') : $_POST['Reservas']['horae'];
            $model->fecha_salida = empty($_POST['Reservas']['fechae']) || empty($_POST['Reservas']['fechas']) ? date("d-m-Y", strtotime(date("d-m-Y") . "+ 1 days")) : $_POST['Reservas']['fechas'];
            ...
            $position = null;
            foreach ($precios_diarios as $key => $data) {
                if ($data['cantidad'] == $cant_dias) {
                    $position = $key;
                }
            }
            ...
            if (!is_null($precioTemporada)) {
                if ($fecha_entrada > strtotime($precioTemporada->fecha_inicio . ' ' . $precioTemporada->hora_inicio) && $fecha_entrada < strtotime($precioTemporada->fecha_fin . ' ' . $precioTemporada->hora_fin)) {
                    $precios_diarios[$position]['precio'] = $precios_diarios[$position]['costo'] + ($cant_dias * $precioTemporada->precio);
                }
            }
